...
pub fn evaluate(state: &State) -> f32 {
    ...
    let mut iter = state.side_one_1.pokemon.into_iter();
    let mut s1_1_used_tera = false;
    while let Some(pkmn) = iter.next() {
        if pkmn.hp > 0 {
            score += evaluate_pokemon(pkmn);
            score += evaluate_hazards(pkmn, &state.side_one_1);
            if iter.pokemon_index == state.side_one_1.active_index {
                for vs in state.side_one_1.volatile_statuses.iter() {
                    match vs {
                ...
                score += get_boost_multiplier(state.side_one_1.attack_boost) * POKEMON_ATTACK_BOOST;
                score += get_boost_multiplier(state.side_one_1.defense_boost) * POKEMON_DEFENSE_BOOST;
                score += get_boost_multiplier(state.side_one_1.special_attack_boost)
                    * POKEMON_SPECIAL_ATTACK_BOOST;
                score += get_boost_multiplier(state.side_one_1.special_defense_boost)
                    * POKEMON_SPECIAL_DEFENSE_BOOST;
                score += get_boost_multiplier(state.side_one_1.speed_boost) * POKEMON_SPEED_BOOST;
            }
        }
    ...
    let mut iter = state.side_one_2.pokemon.into_iter();
    let mut s1_2_used_tera = false;
    while let Some(pkmn) = iter.next() {
        if pkmn.hp > 0 {
            score += evaluate_pokemon(pkmn);
            score += evaluate_hazards(pkmn, &state.side_one_2);
            if iter.pokemon_index == state.side_one_2.active_index {
                for vs in state.side_one_2.volatile_statuses.iter() {
                    match vs {
                        PokemonVolatileStatus::LEECHSEED => score += LEECH_SEED,
                        PokemonVolatileStatus::SUBSTITUTE => score += SUBSTITUTE,
                        PokemonVolatileStatus::CONFUSION => score += CONFUSION,
                ...
                score += get_boost_multiplier(state.side_one_2.attack_boost) * POKEMON_ATTACK_BOOST;
                score += get_boost_multiplier(state.side_one_2.defense_boost) * POKEMON_DEFENSE_BOOST;
                score += get_boost_multiplier(state.side_one_2.special_attack_boost)
                    * POKEMON_SPECIAL_ATTACK_BOOST;
                score += get_boost_multiplier(state.side_one_2.special_defense_boost)
                    * POKEMON_SPECIAL_DEFENSE_BOOST;
                score += get_boost_multiplier(state.side_one_2.speed_boost) * POKEMON_SPEED_BOOST;
            }
        }
        if pkmn.terastallized {
            s1_2_used_tera = true;
    ...
    let mut iter = state.side_two_1.pokemon.into_iter();
    let mut s2_1_used_tera = false;
    while let Some(pkmn) = iter.next() {
        if pkmn.hp > 0 {
            score -= evaluate_pokemon(pkmn);
            score -= evaluate_hazards(pkmn, &state.side_two_1);

            if iter.pokemon_index == state.side_two_1.active_index {
                for vs in state.side_two_1.volatile_statuses.iter() {
                    match vs {
                        PokemonVolatileStatus::LEECHSEED => score -= LEECH_SEED,
                        PokemonVolatileStatus::SUBSTITUTE => score -= SUBSTITUTE,
                        PokemonVolatileStatus::CONFUSION => score -= CONFUSION,
                ...
                score -= get_boost_multiplier(state.side_two_1.attack_boost) * POKEMON_ATTACK_BOOST;
                score -= get_boost_multiplier(state.side_two_1.defense_boost) * POKEMON_DEFENSE_BOOST;
                score -= get_boost_multiplier(state.side_two_1.special_attack_boost)
                    * POKEMON_SPECIAL_ATTACK_BOOST;
                score -= get_boost_multiplier(state.side_two_1.special_defense_boost)
                    * POKEMON_SPECIAL_DEFENSE_BOOST;
                score -= get_boost_multiplier(state.side_two_1.speed_boost) * POKEMON_SPEED_BOOST;
            }
        }
        if pkmn.terastallized {
            s2_1_used_tera = true;
    ...
    let mut iter = state.side_two_2.pokemon.into_iter();
    let mut s2_2_used_tera = false;
    while let Some(pkmn) = iter.next() {
        if pkmn.hp > 0 {
            score -= evaluate_pokemon(pkmn);
            score -= evaluate_hazards(pkmn, &state.side_two_2);

            if iter.pokemon_index == state.side_two_2.active_index {
                for vs in state.side_two_2.volatile_statuses.iter() {
                    match vs {
                        PokemonVolatileStatus::LEECHSEED => score -= LEECH_SEED,
                        PokemonVolatileStatus::SUBSTITUTE => score -= SUBSTITUTE,
                        PokemonVolatileStatus::CONFUSION => score -= CONFUSION,
                ...
                score -= get_boost_multiplier(state.side_two_2.attack_boost) * POKEMON_ATTACK_BOOST;
                score -= get_boost_multiplier(state.side_two_2.defense_boost) * POKEMON_DEFENSE_BOOST;
                score -= get_boost_multiplier(state.side_two_2.special_attack_boost)
                    * POKEMON_SPECIAL_ATTACK_BOOST;
                score -= get_boost_multiplier(state.side_two_2.special_defense_boost)
                    * POKEMON_SPECIAL_DEFENSE_BOOST;
                score -= get_boost_multiplier(state.side_two_2.speed_boost) * POKEMON_SPEED_BOOST;
            }
        }
        if pkmn.terastallized {
            s2_2_used_tera = true;
    ...
    // all these should be the same between both sides so it doesn't matter which one we take them from
    score += state.side_one_1.side_conditions.reflect as f32 * REFLECT;
    score += state.side_one_1.side_conditions.light_screen as f32 * LIGHT_SCREEN;
    score += state.side_one_1.side_conditions.aurora_veil as f32 * AURORA_VEIL;
    score += state.side_one_1.side_conditions.safeguard as f32 * SAFE_GUARD;
    score += state.side_one_1.side_conditions.tailwind as f32 * TAILWIND;
    score += state.side_one_1.side_conditions.healing_wish as f32 * HEALING_WISH;

    score -= state.side_two_1.side_conditions.reflect as f32 * REFLECT;
    score -= state.side_two_1.side_conditions.light_screen as f32 * LIGHT_SCREEN;
    score -= state.side_two_1.side_conditions.aurora_veil as f32 * AURORA_VEIL;
    score -= state.side_two_1.side_conditions.safeguard as f32 * SAFE_GUARD;
    score -= state.side_two_1.side_conditions.tailwind as f32 * TAILWIND;
    score -= state.side_two_1.side_conditions.healing_wish as f32 * HEALING_WISH;
